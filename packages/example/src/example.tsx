...
export const Example: FC = () => {
  ...
  // could look more cleanly if you separate the two cases out into two separate files.
  // one just with runtime code, one with the dedicated plugin.

  // Note: `say` in this example is code-generated by Connect-Query, but if you are already generating a `Service` with Connect-Web, you can access the exact same functionality by calling `createQueryService` directly with your service.  Note that the two types below for `say` and `connectWebSay` are identical:
  say;
  //^? (alias) const say: UnaryHooks<SayRequest, SayResponse>

  const { say: connectWebSay } = createQueryService({ service: ElizaService });
  //           ^? (property) connectWebSay: UnaryHooks<SayRequest, SayResponse>
